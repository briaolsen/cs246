	...
	*/
	public void updateCreatures() {
		...
		for(Creature c : _creatures) {
				...
				if (c instanceof Spawner && ((Spawner) c).isSpawn()) {
					_spawns.add(((Spawner) c).spawnNewCreature());
				}
			}


			Plant p = new Plant();
			Point pt = new Point(_rand.nextInt(_rows), _rand.nextInt(_cols));
			p.setLocation(pt);
			_spawns.add(p);

		_creatures.addAll(_spawns);
		_spawns.clear();
	...
	public void cleanUp() {
		Iterator<Creature> it = _creatures.iterator();
		while(it.hasNext()) {
			Creature s = it.next();
			if (!s.isAlive()) {
				it.remove();
