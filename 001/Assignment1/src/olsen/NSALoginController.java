...
public final class NSALoginController {
	...
	*/
	public static void hashUserPassword(User user) throws Exception {
		...
		// check to see if the password is valid (>8 and contains a digit)
		char[] password = user.getPassword().toCharArray();
		int passwordLength = user.getPassword().length();
		boolean containsDigit = false;

		// check to see if the password contains a digit
		...
		// throw exception if the password is less than 8
		// or does not contain any digits
		if (!containsDigit || passwordLength < 8) {
			throw new WeakPasswordException("Invalid Password " +
					"(must be 8 or more characters and contain at least one digit)");
		}
